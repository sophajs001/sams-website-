            ...
            <ul class="sidebar-menu">
                ...
                @php
                    ...
                    if (\Illuminate\Support\Facades\Schema::hasTable('pages')) {
                        $sidebarPages = \App\Models\Page::orderBy('sort_order')
                            ->orderBy('title')
                            ->get()
                            ->groupBy(fn ($p) => $p->section ?: 'other');
                    }
                    $sectionMeta = [
                        'main' => ['label' => 'Home', 'icon' => 'bi-house'],
                        'about' => ['label' => 'About Us', 'icon' => 'bi-info-square'],
                        'academics' => ['label' => 'Academics', 'icon' => 'bi-mortarboard'],
                        'resources' => ['label' => 'Resources', 'icon' => 'bi-collection'],
                        'info' => ['label' => 'Info', 'icon' => 'bi-megaphone'],
                        'community' => ['label' => 'Community', 'icon' => 'bi-people-fill'],
                    ];
                @endphp

                @foreach($sectionMeta as $key => $meta)
                    @if($sidebarPages->has($key))
                        <li class="menu-item">
                            <a href="{{ route('pages.index') }}#{{ $key }}" class="menu-link">
                                <i class="bi {{ $meta['icon'] }}"></i>
                                <span>{{ $meta['label'] }}</span>
                            </a>
                            <ul class="submenu">
                                @foreach($sidebarPages[$key] as $sp)
                                    <li>
                                        <a href="{{ route('pages.edit', $sp) }}" class="submenu-link">
                                            {{ $sp->title }}
                                        </a>
                                    </li>
                                @endforeach
